...
export async function handleBulkUpdateUsers(params: {
  selectedUsers: AdminUser[]
  payload: Omit<AdminUserBulkUpdatePayload, 'user_ids'>
  actionLabel: string
  message: MessageInstance
  setBulkUpdating: (updating: boolean) => void
  setUsers: (updater: (prev: AdminUser[]) => AdminUser[]) => void
  ...
  const performUpdate = async (twoFactorCode?: string) => {
    ...
    try {
      const { bulkUpdateUsers } = await import('../../../lib/admin')
      const updatedUsers = await bulkUpdateUsers({ user_ids: userIds, ...payload }, twoFactorCode)
      const updatedById = new Map(updatedUsers.map((user) => [user.id, user]))
      setUsers((prev: AdminUser[]) => prev.map((item: AdminUser) => updatedById.get(item.id) ?? item))
      setSelectedUserIds([])
      message.success(`已${actionLabel} ${updatedUsers.length} 个用户`)
      setTwoFactorDialog(null)
  ...
  try {
    if (currentUser?.two_factor_enabled) {
      openTwoFactorDialog('二步验证后批量更新用户', `管理员批量${actionLabel}属于危险操作，请先完成二步验证。`, async (code: string) => {
        try { await performUpdate(code) } catch (err: unknown) { message.error(resolveErrorMessage(err)) }
      })
      return
    }
    await performUpdate()
...
export async function handleBulkDeleteUsers(params: {
  selectedUsers: AdminUser[]
  deleteForm: FormInstance
  editingUser: AdminUser | null
  message: MessageInstance
  ...
  try {
    ...
    const performDelete = async (twoFactorCode?: string) => {
      setDeleting(true)
      try {
        const { bulkDeleteUsers } = await import('../../../lib/admin')
        await bulkDeleteUsers({ user_ids: userIds }, twoFactorCode)
        const deletedIds = new Set(userIds)
        setUsers((prev: AdminUser[]) => prev.filter((item: AdminUser) => !deletedIds.has(item.id)))
        if (editingUser && deletedIds.has(editingUser.id)) closeEditModal()
        setSelectedUserIds([])
    ...
    if (currentUser?.two_factor_enabled) {
      openTwoFactorDialog('二步验证后批量删除用户', '批量删除用户属于危险操作，请先完成二步验证再继续。', async (code: string) => {
        try { await performDelete(code) } catch (err: unknown) { message.error(resolveErrorMessage(err)) }
      })
      return
    }
