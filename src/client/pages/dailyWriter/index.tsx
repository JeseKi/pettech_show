...
export default function DailyWriterPage({
  ...
  const columns: ColumnsType<MatrixRow> = [
    ...
    { title: '行动引导', dataIndex: 'hook', width: 260, ellipsis: true },
    ...
    <section className="growth-create-panel">
      <div className="growth-panel-heading">
        ...
        <Typography.Paragraph>
          稿件任务以选题任务中的选题作为输入。提交后配置会锁定，并沉淀到左侧稿件任务列表。
        </Typography.Paragraph>
      </div>
              ...
              <>
                ...
                {selectedMatrix && (
                  <div className="growth-config-summary">
                    ...
                    <ConfigItem label="选题数量" value={String(Number(selectedMatrix.summary.seed_count ?? selectedMatrix.params.expected_seed_count ?? 0))} />
                  </div>
                )}
              </>
            ),
            ...
            content: (
              <>
                ...
                <div className="growth-writer-config-row">
                  ...
                  <ConfigItem label="多版本稿件" value={`${variantCount} 篇`} />
    ...
    <section className="growth-result-panel">
      <Flex align="flex-start" justify="space-between" wrap="wrap" gap={12}>
        <div className="growth-panel-heading">
          ...
          <Typography.Paragraph>配置已锁定。你可以查看生成结果，或基于同一个选题新建一轮稿件任务。</Typography.Paragraph>
        </div>
        <Space wrap>
          <Button icon={<ReloadOutlined />} onClick={onRefresh}>刷新</Button>
          {result && <Button icon={<DownloadOutlined />} onClick={onDownload}>下载压缩包</Button>}
          <Button type="primary" onClick={onCreateFromCurrent}>基于同一选题新建任务</Button>
        </Space>
      </Flex>
...
function visibleProgressEvents(job: DailyWriterJob) {
  const events = Array.isArray(job.progress?.events) ? job.progress.events : []
  if (!hasTerminalProgressWhileActive(job)) return events
  return events.filter((item) => item.summary !== '任务完成' && item.event !== '失败')
}

function hasTerminalProgressWhileActive(job: Pick<DailyWriterJob, 'status' | 'progress'>): boolean {
...
function ArticlePreviewTabs({
  ...
  const articles = [
    ...
    ...result.variants.map((variant, index) => ({
      ...
      label: variant.angle || `多版本稿件 ${index + 1}`,
