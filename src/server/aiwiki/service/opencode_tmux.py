# -*- coding: utf-8 -*-
"""Run OpenCode inside tmux sessions for long-running AI jobs."""
...
from .constants import LEGACY_PROGRESS_COMPLETE_EVENT, PROGRESS_COMPLETE_EVENT
from .logs import append_log
from .progress import mark_progress_running, read_progress
...
def run_opencode_in_tmux(
    ...
    *,
    ...
    for attempt in range(max_resume_attempts + 1):
        session_id = _read_session_id(workdir) if attempt > 0 else None
...
def _run_opencode_attempt(
    ...
    session_name = _session_name(workdir, title, run_id)
    ...
    _start_tmux_session(session_name, script_path)
    deadline = datetime.now(timezone.utc).timestamp() + global_config.aiwiki_task_timeout_seconds

    while True:
        now = datetime.now(timezone.utc).timestamp()
        if not _read_session_id(workdir) and now >= next_session_probe:
            _persist_session_id(
                workdir,
                title=title,
...
def _read_session_id(workdir: Path) -> str | None:
...
def _persist_session_id(
    ...
    existing = _read_session_id(workdir)
    if existing:
        return existing
...
def _start_tmux_session(session_name: str, script_path: Path) -> None:
    tmux_cmd = [
        "tmux",
        "new-session",
        "-d",
