...
class SocialCardJobDAO(BaseDAO):
    ...
    def get(self, job_id: str) -> SocialCardJob | None:
        return (
            self.db_session.query(SocialCardJob)
            .populate_existing()
            .filter(SocialCardJob.id == job_id)
            .first()
        )
    ...
    def list(
        ...
        *,
        ...
        query = self.db_session.query(SocialCardJob)
        if owner_user_id is not None:
            query = query.filter(SocialCardJob.owner_user_id == owner_user_id)
        if source_daily_writer_job_id:
            query = query.filter(
                SocialCardJob.source_daily_writer_job_id == source_daily_writer_job_id
            )
        return (
            query.order_by(SocialCardJob.created_at.desc(), SocialCardJob.id.desc())
            .offset(offset)
            .limit(limit)
            .all()
        )
    ...
    def update(self, job_id: str, **fields: Any) -> SocialCardJob:
        job = self.get(job_id)
        if job is None:
    ...
    def delete(self, job: SocialCardJob) -> None:
