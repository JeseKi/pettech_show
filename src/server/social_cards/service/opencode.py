...
- 字体优先使用当前工作目录内 `.agents/assets/fonts/` 下的已授权字体文件，例如 `msyh.ttc`、`msyh.ttf`、`msyhbd.ttc`、`MicrosoftYaHei.ttf`、`NotoSansSC-Regular.otf` 或 `NotoSansSC-Bold.otf`；生成 `index.html` 时可通过 `@font-face` 引用这些本地字体，并把中文字体族放在 `font-family` 首位。
