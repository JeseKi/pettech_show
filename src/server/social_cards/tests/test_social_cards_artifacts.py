...
from src.server.social_cards.service.artifacts import prepare_skill


def test_prepare_skill_copies_agent_assets(tmp_path: Path, monkeypatch):
    project_root = tmp_path / "project"
    skill_dir = project_root / ".agents" / "skills" / "guizang-social-card-skill"
    font_dir = project_root / ".agents" / "assets" / "fonts"
    skill_dir.mkdir(parents=True)
    font_dir.mkdir(parents=True)
    (skill_dir / "SKILL.md").write_text("# guizang\n", encoding="utf-8")
