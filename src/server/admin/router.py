...
async def bulk_update_users(
    ...
    try:
        ...
        service.assert_current_admin_not_in_users(users, current_admin)
        update_data = payload.model_dump(exclude={"user_ids"}, exclude_none=True)
        return service.bulk_update_users(db, users, update_data)
    except LookupError:
...
@router.delete(
    ...
    summary="批量删除用户",
    description="管理员批量删除指定用户",
    responses={
        204: {"description": "批量删除成功"},
        400: {"description": "请求参数错误"},
        403: {"description": "无管理员权限"},
        404: {"description": "用户不存在"},
    },
