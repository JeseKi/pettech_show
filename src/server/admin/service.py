...
def assert_current_admin_not_in_users(users: list[User], current_admin: User) -> None:
    if any(user.id == current_admin.id for user in users):
...
def bulk_update_users(db: Session, users: list[User], update_data: dict) -> list[User]:
    ...
    for user in users:
        updated_users.append(update_user(db, user, dict(update_data)))
