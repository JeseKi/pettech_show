# -*- coding: utf-8 -*-
"""Persistence helpers for social card video jobs."""
...
from ..dao import SocialCardVideoJobDAO, parse_json_dict
from ..models import SocialCardVideoJob
from .permissions import can_access_job


def new_job_id(now: datetime) -> str:
    return f"{now.strftime('%Y%m%d%H%M%S')}_{secrets.token_hex(4)}_social_card_videos"
...
def get_accessible_job(db: Session, job_id: str, current_user: Any) -> SocialCardVideoJob:
    if not re.fullmatch(r"\d{14}_[a-f0-9]{8}_social_card_videos", job_id):
...
def update_job(session: Session, job_id: str, **fields: Any) -> None:
    SocialCardVideoJobDAO(session).update(job_id, **fields)
...
def build_session_factory(db: Session) -> sessionmaker[Session]:
    ...
    if not isinstance(bind, Engine):
        raise RuntimeError("无法创建轮播视频任务会话工厂")
...
def write_manifest(workdir: Path, job: SocialCardVideoJob | None) -> None:
    if job is None:
        return
    payload = {
        "id": job.id,
