# -*- coding: utf-8 -*-
"""API serializers for social card video jobs."""
...
from ..schemas import SocialCardVideoJobOut, SocialCardVideoJobSummaryOut


def job_out_from_model(
    job: SocialCardVideoJob, owner_username: str | None = None
) -> SocialCardVideoJobOut:
    return SocialCardVideoJobOut(
        id=job.id,
...
def job_summary_from_model(
    job: SocialCardVideoJob, owner_username: str | None = None
) -> SocialCardVideoJobSummaryOut:
    return SocialCardVideoJobSummaryOut(
        id=job.id,
        owner_user_id=job.owner_user_id,
        owner_username=owner_username,
        source_social_card_job_id=job.source_social_card_job_id,
